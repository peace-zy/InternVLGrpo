...
## News 🚀🚀🚀
- `2025/02/28`: Open a rough version of the GRPO implementation based on TRL, with the code running normally under Internvl2.0 2B, zero1, and zero3. Currently, there are no actual experimental results, since the frequent updates of TRL, there are many parts that need to be modified of Internvl. The offline testing of Intervl shows a significant difference between vLLM and HF. A solution to ensure consistent results has not yet been found. Currently, GRPO does not support vLLM. Since the versions of dependencies like transformers required by Internvl cannot be too high, the TRL source files are directly copied into the project.
