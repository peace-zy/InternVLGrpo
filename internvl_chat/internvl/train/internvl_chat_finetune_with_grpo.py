...
class ModelArguments:
    ...
    save_model_to_multi_node: bool = field(
        default=False,
        metadata={'help': 'Set to True to save the model to all nodes.'},
    )
...
class DataTrainingArguments:
    ...
    force_shuffle: bool = field(
        default=True,
        metadata={'help': 'Whether to force shuffle the dataset. Default is True.'},
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        if self.force_shuffle:
            logger.info('[Dataset] Shuffling the dataset')
...
def build_datasets(
    ...
    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        root = ds_collections[ds_name].get('root', '')
        annotation = ds_collections[ds_name].get('annotation', '')
        if not root or not annotation:
            logger.info(f'Skip dataset {ds_name} due to missing root or annotation')
            continue
        ...
        logger.info(f'Loading dataset: {ds_name} with repeat_time: {repeat_time}')
        dataset = LazySupervisedDataset(
            ...
            force_shuffle=data_args.use_packed_ds or data_args.force_shuffle,
            random_seed=ds_idx,
        )
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, InternvlGRPOConfig))
    ...
    if model_args.model_name_or_path is not None:
        ...
        if model_args.save_model_to_multi_node:
    ...
    # 存储模型为了多卡训练
    if model_args.save_model_to_multi_node:
        output_path = '/chubao/tj-train-ssd-21/zhangyan/models/OpenGVLab/InternVL2-8B-add-token-new'
        logger.info('Saving InternVLChatModel to multi node ...')
        model.save_pretrained(output_path)
        logger.info('Finished saving InternVLChatModel to multi node ...')
